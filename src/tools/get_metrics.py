...
class GetMetricsTool(Tool):
    ...
    async def execute(self, params: dict) -> dict:
        ...
        try:
            ...
            logger.info("메트릭 조회 시작 - RSID: %s, 페이지: %d, 제한: %d", 
            ...
            while True:
                async with aiohttp.ClientSession() as session:
                    ...
                    url = f"https://analytics.adobe.io/api/{self.auth.company_id}/metrics"
                    request_params = {
                    ...
                    async with session.get(url, headers=headers, params=request_params) as response:
                        ...
                        if len(all_metrics) >= validated_params.max_results:
                            logger.info("최대 결과 수(%d) 도달", validated_params.max_results)
                            break
