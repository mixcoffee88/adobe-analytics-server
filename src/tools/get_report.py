...
def parse_date_range(date_range: str) -> str:
    ...
    start_iso = start_date.replace(hour=0, minute=0, second=0, microsecond=0).isoformat() + ".000"
    end_iso = end_date.replace(hour=0, minute=0, second=0, microsecond=0).isoformat() + ".000"
    
    return f"{start_iso}/{end_iso}"

class GetReportParams(BaseModel):
    """리포트 파라미터"""
    date_range: str = Field(..., description="날짜 범위 (예: last_3_days, this_week, last_month)")
    metrics: List[str] = Field(..., description="지표 목록")
    dimension: Optional[str] = Field(default="daterangeday", description="차원 (기본값: daterangeday)")
